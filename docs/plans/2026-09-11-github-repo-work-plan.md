...
## Status 2026-09-11 (later): E2E passed; follow-ups added

Manual E2E succeeded (agent opened vise-sh/vise#1, outcome `pr_opened`).
Follow-ups implemented afterwards, statically reviewed, but NOT yet
compiled/tested (harness outage): (a) `GITHUB_TOKEN` alias alongside
`GH_TOKEN` in acp.rs `resolve_command` + 4 new unit tests; (b) startup
orphan-workspace sweep in vise-host main.rs honoring `.keep` markers written
by all keep paths. **Pending: `cargo test -p vise-host && cargo build
--workspace`.** Everything remains uncommitted for user review.
