...
async fn run_session(
        ...
        && run_ok
    {
        ...
        match github::detect_outcome(prepared, pr_lookup).await {
            Ok(detected) => {
                // A follow-up pushes to a branch whose PR already exists: that
                // is an update, not a new PR, and is never tracked separately.
                let kind = if detected.kind == "pr_opened" && session.parent_session_id.is_some() {
...
async fn prepare_github(
    ...
    session.input = if session.parent_session_id.is_some() {
        // Follow-up: the base branch is the PR's head branch, so pushing to it
        // updates the existing pull request.
        format!(
            "You are working in a clone of {repo}, checked out on branch {base}, which already \
             has an open pull request. Complete the task below. When done: commit your work with \
             clear messages and push to the same branch ({base}) so the existing pull request \
             updates. Do not open a new pull request and do not force-push. Report the PR URL in \
             your final message.\n\n{input}",
