...
fn resolve_command(harness: &str, gh_token: Option<&str>) -> anyhow::Result<String> {
    ...
    Ok(match gh_token {
        // GITHUB_TOKEN alias: some tools (actions toolkits, older gh) only read it.
        Some(token) => format!("env GH_TOKEN={token} GITHUB_TOKEN={token} {base}"),
        None => base.to_string(),
    })
}
...
mod tests {
    ...
    #[test]
    fn resolve_command_injects_both_token_vars() {
        let command = resolve_command("claude-code", Some("sekrit")).unwrap();
        assert!(command.starts_with("env GH_TOKEN=sekrit GITHUB_TOKEN=sekrit "));
        assert!(command.ends_with("npx -y @agentclientprotocol/claude-agent-acp@latest"));
    }
    ...
    #[test]
    fn resolve_command_rejects_unknown_harness() {
        assert!(resolve_command("mystery", None).is_err());
    }
